...
LEFT, RIGHT, UP, DOWN, STOP, HORN= False, False, False, False, False, False
...
while running:
    ...
    print(LEFT, RIGHT, UP, DOWN, STOP, HORN)
    try:
        if LEFT or RIGHT or UP or DOWN or STOP or HORN:
            if LEFT:
                connector.write(bytes('L', 'utf-8'))
                LEFT = False
            if RIGHT:
                connector.write(bytes('R', 'utf-8'))
                RIGHT = False
            if UP:
                connector.write(bytes('U', 'utf-8'))
                UP = False

            if DOWN:
                connector.write(bytes('D', 'utf-8'))
                DOWN = False

            if STOP:
                connector.write(bytes('S', 'utf-8'))
                STOP = False

            if HORN:
                connector.write(bytes('H', 'utf-8'))
                HORN = False
